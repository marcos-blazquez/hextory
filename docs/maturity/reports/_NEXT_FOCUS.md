...
1. R-0042 is a flat reconfirm at **80.9** (Δ0 vs R-0041). Main evidence tip before this pulse land is `b46485b` (R-0041 / PR #41); product tip remains `65a46c8`. Local design-gates pass; pytest is **115 passed / 1 skipped**. Do not invent lifts from docs-only tip CI.
...
5. Prefer GitHub `main` + fresh SSH clone as evidence SoT. Mac is disconnected; do not infer product progress from it. Public `.github/` remains workflows-only — no issue/PR templates.
6. Always run `python scripts/ci_design_gates.py` + pytest. Overall <90 means continue weekday hourly pulses; ≥90 pauses the routine.
...
8. Memory note for next run: R-0030's aspect-9 lift was followed by R-0031…R-0042 flat reconfirms. Only operated OBS, fuller AWS deploy evidence, contribution templates, or Approved workflow-SDD evidence can move scores.
